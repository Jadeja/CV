...
//COLLECT POST data//For loop must be closed at the top of your script before carrying on with the query
	...
	$ds =  $_SESSION["UserID"];
		...
		else
		{
			...
			$sql1 	= $dbh->prepare("select description from advises where title = :title"); // to check if record exists or not			
			$sql1->bindParam(':title',$_GET['title'],PDO::PARAM_STR);
			$output = $sql1->execute(); 
			if(!$output)
			{
				$msg =  "Tips not available" ;			
				$dbh = null;
				echo json_encode(array('success'=>false,'message' => $msg));
				exit();
			}
			else
			{	$output = $sql1->fetchAll();
				$msg  = "successful" ;			
				echo json_encode(array('success'=>true,'desc'=>$output[0]['description'],'message' =>$msg));
				$dbh  = null;
				exit();				
			}
		}
